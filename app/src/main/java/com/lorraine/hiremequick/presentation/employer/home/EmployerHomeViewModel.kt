...
class EmployerHomeViewModel @Inject constructor(
    ...
    private fun observeAllJobs() {
        allLogsJob = viewModelScope.launch {
            ...
            FirebaseAttachmentLogRepo.getAllJobsPostings(employerId = currentUserId).debounce(2000).collect { result ->
    ...
    private fun observeFilteredAttachmentLogs(zonedDateTime: ZonedDateTime) {
        filteredLogsJob = viewModelScope.launch {
            ...
            FirebaseAttachmentLogRepo.getFilteredJobPostings(zonedDateTime = zonedDateTime, studentId = currentUserId)
                .collect { result ->
                    jobPostings.value = result
                }
        }
